...
> 输入观点或材料，后台核验重要变化，并把你的新想法带入下一轮。

当前默认产品是 **V2 持续研究工作台**（`/v2`）。旧的研报核验流程仍可从「研报核验」入口查看，作为历史回放，不再作为默认写入路径。

V2 运行依据是 SQLite；Markdown 只是导出。建议使用新的 `data-v2-local` 目录，保留既有研究数据。

详细说明见 [docs/v2/README.md](docs/v2/README.md)。
